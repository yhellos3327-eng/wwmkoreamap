...
const matchesShortcut = (event, shortcut) => {
    const keyMatches = event.key.toLowerCase() === shortcut.key.toLowerCase() ||
        event.code.replace('Key', '').toLowerCase() === shortcut.key.toLowerCase() ||
        event.code.replace('Digit', '').toLowerCase() === shortcut.key.toLowerCase();
...
const updateKeyboardDisplay = () => {
    ...
    keyButtons.forEach(btn => {
        ...
        const assignedAction = findActionForKey(key, selectedModifiers.ctrl, selectedModifiers.shift, selectedModifiers.alt);
...
const updateModifierButtons = () => {
    ...
    document.querySelectorAll('.keyboard-key[data-modifier]').forEach(key => {
        ...
        if (mod === 'shift') key.classList.toggle('active', selectedModifiers.shift);
...
const showActionSelector = (key) => {
    ...
    const currentAction = findActionForKey(key, selectedModifiers.ctrl, selectedModifiers.shift, selectedModifiers.alt);
    ...
    AVAILABLE_ACTIONS.forEach(action => {
        ...
        item.addEventListener('click', () => {
            assignActionToKey(action.id, key, selectedModifiers.ctrl, selectedModifiers.shift, selectedModifiers.alt);
            dropdown.remove();
            selectedKey = null;
            updateKeyboardDisplay();
        });
