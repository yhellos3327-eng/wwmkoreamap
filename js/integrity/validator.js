...
/**
 * @param {any} data
 * @returns {{valid: boolean, issues: string[], fatal: boolean}}
 */
 ...
 * @returns {{valid: boolean, legacy?: boolean, futureVersion?: boolean, message?: string}}
 */
  ...
  for (let i = 0; i < markers.length; i++) {
    ...
    if (error) {
      ...
      if (result.issues.length < 3) {
        result.issues.push(
          `항목 ${i + 1} (ID: ${markerId || "unknown"}): ${error}`,
        );
      }
    } else {
      result.validCount++;
...
export const validateFavorites = (favorites, context = {}) => {
  ...
  for (let i = 0; i < favorites.length; i++) {
    ...
    if (
      ...
      favId &&
      hasMarkerData &&
      !context.validMarkerIds.has(favId)
    ) {
      error = "존재하지 않는 마커 ID";
    }
    ...
    if (!validation.valid) {
      if (
        ...
        const cleanedValue = value.filter(
          (item) => !validation.invalidItems.includes(item),
        );
...
export const validateBackupData = (data, context = {}, options = {}) => {
  ...
  const completedResult = validateCompletedMarkers(
  ...
  if (!completedResult.skip && !completedResult.valid) {
    const errorRate =
      completedResult.invalidCount /
      (completedResult.validCount + completedResult.invalidCount);

    const severity =
      options.mode === "strict" || errorRate > 0.1
        ? SEVERITY.ERROR
        : SEVERITY.WARNING;
  ...
  if (data.settings) {
    ...
    for (const issue of settingsResult.issues) {
      const severity =
        options.mode === "strict" ? SEVERITY.ERROR : SEVERITY.WARNING;
      addIssue(result, {
  ...
  result.summary.passed =
    result.summary.total - result.summary.failed - result.summary.warnings;
