...
export const runIntegrityCheck = async (data, onProceed, options = {}) => {
    ...
    if (!data || typeof data !== 'object') {
        ...
        showResult('error', '복원 불가', '데이터 형식에 치명적인 오류가 있습니다.', '');
    ...
    const completedResult = await runCompletedCheck(data.completedMarkers, context, opts);
    ...
    if (completedResult.skip) {
        ...
        consoleLog(`  ✓ 완료 목록 검사 통과: ${completedResult.validCount}개`, 'success');
        updateStep('completed', CHECK_STATUS.SUCCESS, `${completedResult.validCount}개`);
    } else if (completedResult.severity === SEVERITY.WARNING) {
        consoleLog(`  ⚠ 완료 목록 검사 경고: ${completedResult.invalidCount}개 무효`, 'warning');
        updateStep('completed', CHECK_STATUS.WARNING, `${completedResult.validCount}개 (${completedResult.invalidCount}개 경고)`);
    } else {
        consoleLog(`  ✗ 완료 목록 검사 실패: ${completedResult.invalidCount}개 오류`, 'error');
        updateStep('completed', CHECK_STATUS.ERROR, `${completedResult.invalidCount}개 오류`);
        report.canRestore = false;
    }
    ...
    if (favoritesResult.skip) {
        ...
        consoleLog(`  ✓ 즐겨찾기 검사 통과: ${favoritesResult.validCount}개`, 'success');
        updateStep('favorites', CHECK_STATUS.SUCCESS, `${favoritesResult.validCount}개`);
    } else {
        consoleLog(`  ⚠ 즐겨찾기 검사 경고: ${favoritesResult.invalidCount}개 무효`, 'warning');
        updateStep('favorites', CHECK_STATUS.WARNING, `${favoritesResult.validCount}개 (경고)`);
    }
    ...
    const settingsResult = await runSettingsCheck(data.settings, context, opts, data);
    ...
    if (settingsResult.skip) {
        ...
        updateStep('settings', CHECK_STATUS.SUCCESS, `${settingsResult.count}개 항목`);
    } else if (opts.mode === 'soft') {
        consoleLog(`  ⚠ 설정 검사 경고: ${settingsResult.invalidCount}개 문제`, 'warning');
        updateStep('settings', CHECK_STATUS.WARNING, `${settingsResult.invalidCount}개 경고`);
    } else {
        consoleLog(`  ✗ 설정 검사 실패: ${settingsResult.invalidCount}개 오류`, 'error');
        updateStep('settings', CHECK_STATUS.ERROR, `${settingsResult.invalidCount}개 오류`);
        report.canRestore = false;
    }
    ...
    if (settingsResult.cleaned && settingsResult.cleaned.length > 0) {
        ...
        consoleLog(`> 자동 정리: ${settingsResult.cleaned.length}개 항목 정리됨`, 'warning');
    }
...
const runFavoritesCheck = async (favorites, context) => {
    ...
    return { valid: invalidCount === 0, validCount, invalidCount, severity: invalidCount > 0 ? SEVERITY.WARNING : null };
};
...
const runSettingsCheck = async (settings, context, opts, data) => {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (key.startsWith('activeCats')) {
            if (!Array.isArray(value)) {
                ...
                const invalidItems = value.filter(item => typeof item !== 'string' || !/^\d+$/.test(item));
            ...
            if (!Array.isArray(value)) {
                ...
                const invalidRegions = value.filter(r => typeof r !== 'string' || !context.validRegions.has(r));

                if (invalidRegions.length > 0) {
                    if (opts.autoClean) {
                        const validRegions = value.filter(r => typeof r === 'string' && context.validRegions.has(r));
                        ...
                        consoleLog(`    [${key}]: ${invalidRegions.length}개 유효하지 않은 지역 자동 제거됨`, 'warning');
                        msg = `자동 정리됨: ${invalidRegions.length}개 제거`;
                    } else {
                        isValid = false;
                        const preview = invalidRegions.slice(0, 5).join(', ');
                        const suffix = invalidRegions.length > 5 ? ` 외 ${invalidRegions.length - 5}개` : '';
        ...
        } else if (['showAd', 'showComments', 'hideCompleted', 'enableClustering', 'closeOnComplete'].includes(key)) {
    ...
    return {
        ...
        issues,
        cleaned,
        severity: invalidCount > 0 ? (opts.mode === 'strict' ? SEVERITY.ERROR : SEVERITY.WARNING) : null
    };
};

const hasWarnings = (report) => {
    return Object.values(report.steps).some(step =>
        step.severity === SEVERITY.WARNING || step.status === 'warning'
    );
};
...
const collectIssues = (report) => {
    ...
    return issues.slice(0, 10).map(i => `• ${i}`).join('<br>');
};
