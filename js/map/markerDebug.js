...
export const logMarkerDebugInfo = (item, catId, finalRegionName, lat, lng) => {
  ...
  console.groupCollapsed(
    `%c📍 [${item.id}] ${item.name}`,
    "font-size: 14px; font-weight: bold; color: #ffbd53; background: #222; padding: 4px 8px; border-radius: 4px;",
  );
  console.table(debugInfo);

  if (state.rawCSV && state.parsedCSV && state.parsedCSV.length > 0) {
    console.groupCollapsed(
      "%c📄 CSV Source Data Available",
    ...
    let rowIndex =
      keyIdx !== -1
        ? state.parsedCSV.findIndex((r) => r[keyIdx] == item.id)
        : -1;

    if (rowIndex === -1 && keyIdx !== -1) {
      rowIndex = state.parsedCSV.findIndex(
        (r) => r[keyIdx] === item.name || r[keyIdx] === item.name?.trim(),
      );
    }
