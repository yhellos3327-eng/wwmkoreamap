...
export const updateToggleButtonsState = () => {
    ...
    if (btnToggleCat) {
        ...
        btnToggleCat.innerHTML = allCatActive ? `${EYE_OFF_SVG_STYLED} 모두 끄기` : `${EYE_OPEN_SVG_STYLED} 모두 켜기`;
        btnToggleCat.classList.toggle('off', !allCatActive);
    }
    if (btnToggleReg) {
        const allRegActive = state.activeRegionNames.size === state.uniqueRegions.size;
        btnToggleReg.innerHTML = allRegActive ? `${EYE_OFF_SVG_STYLED} 모두 끄기` : `${EYE_OPEN_SVG_STYLED} 모두 켜기`;
