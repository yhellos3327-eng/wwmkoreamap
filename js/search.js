...
const renderSearchResults = (term, searchInput, searchResults) => {
    ...
    if (matchedRegions.length > 0) {
        ...
        matchedRegions.forEach(region => {
            const markerCount = Array.from(state.allMarkers.values())
                .filter(m => m.region === region).length;
    ...
    if (matchedCategories.length > 0) {
        matchedCategories.forEach(cat => {
            ...
            const totalCount = Object.values(regionCounts).reduce((sum, c) => sum + c, 0);
            ...
            displayRegions.forEach(region => {
                ...
                const isActive = state.activeCategoryIds.has(catId) && state.activeRegionNames.has(region);
    ...
    searchResults.querySelectorAll('.search-embed-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            if (type === 'region') {
                ...
                const region = item.dataset.region;
                handleCategoryRegionClick(categoryId, region, searchInput, searchResults);
            }
        });
    });
...
// 카테고리 + 지역 클릭 핸들러
const handleCategoryRegionClick = (categoryId, region, searchInput, searchResults) => {
