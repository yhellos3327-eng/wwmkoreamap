...
const translateWithChromeBuiltin = async (item, btn) => {
  try {
    const { translateGameItem, isChromeBuiltinTranslationSupported } =
      await import("./chromeTranslator.js");

    if (!isChromeBuiltinTranslationSupported()) {
      throw new Error(
        "Chrome 내장 번역이 지원되지 않습니다. Chrome 138 이상이 필요합니다.",
      );
    }
...
export const translateItem = async (itemId, translateType = "ai") => {
  ...
  const popupContainer = document.querySelector(
    `.popup-container[data-id="${itemId}"]`,
  );
  const btn = popupContainer?.querySelector(
    translateType === "chrome" ? ".btn-translate-chrome" : ".btn-translate-ai",
  );
...
export const calculateTranslationProgress = () => {
  ...
  if (statsText)
    statsText.textContent = `${translatedCount} / ${totalItems} 항목 번역됨`;
};
