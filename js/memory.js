...
export class MemoryManager {
    ...
    setDebug(enabled) {
        ...
        console.log(`%c[MemoryManager] Debug Mode: ${enabled ? 'ON' : 'OFF'}`, 'color: #ff00ff; font-weight: bold;');
        if (enabled) {
            this.logStats();
        }
    }
    ...
    track(target, label, cleanupCallback = null) {
        ...
        if (this.debugMode) {
            console.log(`[MemoryManager] 🟢 Tracking: ${label} (Total Active: ${this.activeCount})`);
        }
    }
    ...
    _cleanup(heldValue) {
        ...
        if (this.debugMode) {
            ...
            console.log(`[MemoryManager] 🗑️ Garbage Collected: ${heldValue.label} (lived for ${duration}ms) | Remaining: ${this.activeCount}`);
        }

        if (heldValue.cleanupCallback && typeof heldValue.cleanupCallback === 'function') {
            try {
                heldValue.cleanupCallback();
            } catch (e) {
                console.error(`[MemoryManager] Error in cleanup callback for ${heldValue.label}:`, e);
            }
        }
    }
