...
export const enableDevMode = () => {
  ...
  const createRegionEditorUI = () => {
    ...
    document.getElementById("btn-toggle-region-mode").onclick =
      toggleRegionMode;
  ...
  const startNewPolygon = () => {
    clearPolygon();
    currentPolygon = L.polygon([], { color: "red", weight: 2 }).addTo(
      state.map,
    );
  };
  ...
  const onMarkerClick = (e) => {
    ...
    const item = Array.from(state.allMarkers.values()).find(
      (m) => m.marker === marker,
    );
    if (!item) return;
    ...
    if (elName)
      elName.value = originalItem ? originalItem.name : item.originalName;
    if (elDesc)
      elDesc.value = originalItem ? originalItem.description || "" : item.desc;
    if (elCategory)
      elCategory.value = originalItem ? originalItem.category : item.category;
    ...
    updateOutput();
    console.log(
      `%cEditing Item: ${item.originalName} (${item.id})`,
      "color: yellow",
    );
  };
  ...
  if (categorySelect) {
    categorySelect.addEventListener("change", (e) => {
      ...
      const selectedCat = state.mapData.categories.find(
