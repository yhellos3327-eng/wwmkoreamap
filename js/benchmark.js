...
const runStressTest = async (autoCompare = false) => {
  ...
  for (const mode of modes) {
    ...
    log(
      `${mode ? "GPU" : "CPU"} Analysis: Avg ${avg} FPS, 1% Low ${low} FPS`,
      "success",
    );
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  setState(
    "activeCategoryIds",
    new Set(state.mapData.categories.map((c) => c.id)),
  );
...
document
  .getElementById("compare-bench")
  .addEventListener("click", () => runStressTest(true));
