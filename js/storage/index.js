...
import { SCHEMA, getSchema, validateValue, getCurrentSnapshotKeys, transformForSave } from './schema.js';
import {
    saveSnapshot,
    getSnapshot,
    getSnapshotInfo,
...
const safeSave = (key, value, isJSON = true) => {
    ...
    const result = isJSON ? core.setJSON(key, transformed) : core.set(key, transformed);
...
export const persistState = {
    ...
    loadFavoritesForMap: (mapKey) => core.getJSON(STORAGE_KEYS.favorites(mapKey), []),
    ...
    saveSettings: (settings) => {
        if (settings.aiProvider !== undefined) core.set(STORAGE_KEYS.AI_PROVIDER, settings.aiProvider);
        if (settings.apiKey !== undefined) encoded.set(STORAGE_KEYS.API_KEY, settings.apiKey);
        if (settings.openaiKey !== undefined) encoded.set(STORAGE_KEYS.OPENAI_KEY, settings.openaiKey);
        if (settings.claudeKey !== undefined) encoded.set(STORAGE_KEYS.CLAUDE_KEY, settings.claudeKey);
        if (settings.apiModel !== undefined) core.set(STORAGE_KEYS.API_MODEL, settings.apiModel);
        if (settings.regionColor !== undefined) core.set(STORAGE_KEYS.REGION_COLOR, settings.regionColor);
        if (settings.regionFillColor !== undefined) core.set(STORAGE_KEYS.REGION_FILL_COLOR, settings.regionFillColor);
        if (settings.hideCompleted !== undefined) core.set(STORAGE_KEYS.HIDE_COMPLETED, settings.hideCompleted);
        if (settings.enableClustering !== undefined) core.set(STORAGE_KEYS.ENABLE_CLUSTERING, settings.enableClustering);
        if (settings.showComments !== undefined) core.set(STORAGE_KEYS.SHOW_COMMENTS, settings.showComments);
        if (settings.closeOnComplete !== undefined) core.set(STORAGE_KEYS.CLOSE_ON_COMPLETE, settings.closeOnComplete);
        if (settings.showAd !== undefined) core.set(STORAGE_KEYS.SHOW_AD, settings.showAd);
    },
